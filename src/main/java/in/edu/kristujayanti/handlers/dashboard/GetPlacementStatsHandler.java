...
public class GetPlacementStatsHandler implements Handler<RoutingContext> {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(GetPlacementStatsHandler.class);
    ...
    public void handle(RoutingContext routingContext) {
        ...
        try {
            JsonArray stats = dashboardService.getPlacementStats();
            ResponseUtil.createResponse(
                    response,
                    ResponseType.SUCCESS,
                    StatusCode.TWOHUNDRED,
                    new JsonObject().put("data", stats),
                    new JsonArray().add("Placement stats fetched successfully")
            );
        } catch (Exception e) {
            LOGGER.error("Error fetching placement stats", e);
            ResponseUtil.createResponse(
                    response,
                    ResponseType.ERROR,
